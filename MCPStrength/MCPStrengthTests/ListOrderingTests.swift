//
//  ListOrderingTests.swift
//  MCPStrengthTests
//
//  Covers ListOrdering.move — the rule that `index` is the final
...
struct ListOrderingTests {
    ...
    @Test func sameListMoveDown() {
        let list = [a, b, c, d]
        let result = ListOrdering.move(a, from: list, to: list, at: 2)
    ...
    @Test func indexBeyondEndIsClamped() {
        let result = ListOrdering.move(a, from: [a, b], to: [x, y], at: 99)
        #expect(result.source == [b])
        #expect(result.destination == [x, y, a])
    }

    @Test func emptyDestination() {
        let result = ListOrdering.move(a, from: [a], to: [], at: 0)
    ...
    @Test func idAbsentFromSourceLeavesBothUnchanged() {
        let source = [b, c]
        let destination = [x, y]
        let result = ListOrdering.move(a, from: source, to: destination, at: 0)
